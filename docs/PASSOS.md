## O que foi feito até agora:

# [x] b - feito
Coleta de Reviews no site Beleza na Web - Utilizamos como parametro 4-5 estrelas: Positivo; 3 estrelas: neutro; 1-2 estrelas: negativo. Feito um csv com os dados coletados.

## [x] c - feito
Utilizando NLTK.stopwords()
Limpeza de palavaras irrelevantes como artigos, preposições, conjunções, pronomes e palavras de uso frequente como: "isso", "aquilo", "então", "porque"

## [x] d - feito
Etapa de dividir cada avaliação em palavras (tokens). Gera um CSV demonstrando a tokenização com o arquivo `tokenizacao_demonstracao.csv`.

## [x] e - feito
A partir da base da etapa anterior, dividir os dados em duas partes, treinamento e teste utilizando holdout estratificado

## [] f
Estapa do relatório
